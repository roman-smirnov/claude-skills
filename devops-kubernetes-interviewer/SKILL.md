...
> Non-Kubernetes domains (Terraform, Helm, ArgoCD, AWS, reliability, security) use parallel rubrics with the same interview mechanics.
...
5. **Kubernetes Operations** — Troubleshooting, incident response, performance tuning, cluster management
...
- **Terraform-only** — IaC fundamentals, state management, modules
...
1. **Terraform & IaC**
2. **Kubernetes fundamentals & troubleshooting** — grounded in `k8s-troubleshooter@devops-skills` (see `checklists/kubernetes.md`)
...
### Kubernetes Round Enhancement (from `k8s-troubleshooter@devops-skills`)

The Kubernetes round receives additional depth from the `k8s-troubleshooter@devops-skills` composite. When asking Kubernetes questions:

- Reference the **systematic troubleshooting workflow**: gather context → initial triage → deep dive → root cause → remediation → verify & monitor.
- Use **common issues** from `k8s-troubleshooter@devops-skills` as the basis for "What breaks if..." and "How would you debug..." questions: ImagePullBackOff, CrashLoopBackOff, Pending Pods, OOMKilled, Node NotReady, Disk Pressure, pod-to-pod communication failures, PVC Pending, volume mount failures, resource quota exceeded, CPU throttling, RBAC denied.
- Expect candidates to demonstrate **incident response awareness**: severity classification (SEV-1 through SEV-4), structured response phases (detection → triage → investigation → resolution → post-incident), and blameless post-mortems.
- Probe **performance troubleshooting** knowledge: latency diagnosis, CPU/memory profiling, network performance, storage I/O, cluster-wide performance (control plane, scheduler, etcd).
- Evaluate **Helm troubleshooting** skills: stuck releases, upgrade failures, values debugging, hook lifecycle issues, dependency management.
- Score higher when candidates use **systematic diagnostic approaches** rather than ad-hoc commands. Candidates who follow the k8s-troubleshooter workflow (cluster health → namespace check → pod diagnosis) demonstrate operational maturity.
...
- Systematic troubleshooting workflow: [demonstrated / partial / not demonstrated]
- Incident response awareness: [strong / adequate / needs work]
- Common issue diagnosis: [X/12 common issues covered confidently]
- Performance troubleshooting depth: [strong / adequate / needs work]
- Helm troubleshooting: [strong / adequate / needs work]
...
- `Bash`: `kubectl top` (metrics, read-only)
- `Bash`: `kubectl auth can-i` (RBAC check, read-only)
- `Bash`: `kubectl cluster-info`, `kubectl version`
...
12. **Apply k8s-troubleshooter depth in Kubernetes round.** When scoring Kubernetes answers, evaluate whether the candidate demonstrates the systematic troubleshooting methodology from `k8s-troubleshooter@devops-skills`: structured diagnosis, awareness of common failure patterns, incident severity classification, and performance analysis skills.
