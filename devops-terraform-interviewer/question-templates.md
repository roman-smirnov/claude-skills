...
*Grounded in `iac-terraform@devops-skills` standards.*
...
17. "Explain the module structure convention: why must every module have `main.tf`, `variables.tf`, `outputs.tf`, and `versions.tf`? What does `iac-terraform@devops-skills` say about this?"
...
20. "Your `{module}` module has variables without descriptions. According to IaC best practices, why is this a problem?"
...
1. "Your EKS node group in `{path}` is set to `{config_detail}` nodes. One node becomes NotReady. What happens to Pods on that node, and how does Kubernetes respond?"
2. "The `vpc-cni` addon configured in `{path}` runs out of IP addresses in the subnet. How does this manifest and what are your options?"
3. (generic) "A Deployment has no resource requests or limits set. What are the consequences during a node resource crunch?"

### How Would You Debug...

4. "A Pod is stuck in `CrashLoopBackOff`. Walk me through your debugging steps using only `kubectl` commands."
5. "Services in namespace A cannot reach services in namespace B. What do you check first, second, third?"
6. "Your EBS-backed PVC is stuck in `Pending`. Given that you have the `aws-ebs-csi-driver` addon in `{path}`, what are the likely causes?"

### Tradeoff

7. "You use `t3a.medium` instances for your EKS nodes. What are the tradeoffs vs `t3.medium`, `m6i.large`, or Graviton-based instances?"
8. "Public + private endpoint access is enabled for your EKS cluster in `{path}`. Why not private-only? What are the security implications?"
9. (generic) "When would you use a DaemonSet vs a Deployment with node affinity?"

### Technology Comparison

10. "[DEFINE] What is EKS? Why EKS instead of self-managed Kubernetes (kubeadm), ECS, or k3s? What does the managed control plane give you that self-managed doesn't?"
11. "[DEFINE] What is VPC-CNI? You use the AWS VPC-CNI plugin in `{path}`. Why VPC-CNI instead of Calico, Flannel, or Cilium? How does VPC-CNI assign Pod IP addresses differently?"
12. "[DEFINE] What is the EBS CSI Driver? Why use the CSI driver addon instead of the in-tree EBS provisioner? What is IRSA and why does the CSI driver need it?"

### Best Practice Pillar

13. **(Scalability)** "Your EKS node group is fixed at 3 nodes in `{path}`. What is a Cluster Autoscaler vs Karpenter? Why don't you have auto-scaling enabled, and what are the risks?"
14. **(Availability)** "You have 2 AZs with 3 nodes. What happens if one AZ goes down? How many nodes survive? Can your workloads still run? What would you change for higher availability?"
15. **(Security)** "What are Kubernetes RBAC, Pod Security Standards, and Network Policies? Which of these are implemented in your cluster? What attack vectors remain unaddressed?"

### Bonus Pool

16. "Explain the difference between liveness, readiness, and startup probes. When would you use each?"
17. "What is a PodDisruptionBudget and when is it critical?"
18. "How does CoreDNS resolution work for `service.namespace.svc.cluster.local`?"
...
1. "A Helm release upgrade fails mid-way. What state is the release in, and what are your recovery options?"
...
6. (generic) "You suspect a compromised container in your cluster. What do you do in the first 5 minutes?"
